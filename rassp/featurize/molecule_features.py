    ...
    def __call__(self, mol, formulae=None, masses=None):
        ...
        if self.enumerator_type == "multipeak":
            ...
            masses[:, :, 1] = masses[:, :, 1] / np.clip(
                np.sum(masses[:, :, 1], axis=1).reshape(-1, 1), a_min=0.01, a_max=1.0
            )
        ...
        masses = np.pad(masses, ((0, self.max_formulae - src_masses_n), (0, 0), (0, 0)))
        ...
        if self.featurize_mode == "numerical":
            ...
            formulae_feat = np.pad(
                formulae_feat, [(0, self.max_formulae - formulae_feat.shape[0]), (0, 0)]
            )

            return formulae_feat, masses

        elif self.featurize_mode == "onehot":
            formulae_feat = np.zeros((self.max_formulae, self.oh_max), dtype=np.float32)
