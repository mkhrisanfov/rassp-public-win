...
def enumerate_multinomial(N, p, cutoff_thold=0.0001):
    ...
    return list(
        zip(probs_sorted[:thold_idx], [tuple(a) for a in counts[sort_idx][:thold_idx]])
    )
...
def create_cpp_lut(filename):
    ...
    FORMULA_SIZES = {1: 60, 6: 60, 7: 60, 8: 60, 9: 60, 15: 60, 16: 60, 17: 60}

    formula_element_peaks = {}
    for an in ATOMICNOS:
        formula_element_peaks[an] = {
            num: get_peaks_for_elt_num(an, num) for num in range(1, FORMULA_SIZES[an])
        }
...
def peaks_via_sampling(formula, SAMPLE_N=100000):
    ...
    for atomicno, num in formula.items():
        ...
        a = np.random.choice(
            [i[2] for i in isos], p=[i[1] for i in isos], size=(SAMPLE_N, num)
        )
        masses += a.sum(axis=1)
    return masses
