...
class BreakAndRearrange:
    ...
    def __call__(
        self, mol, num_samples=None, max_samples=None, seed=0, return_subsets=False
    ):
        np.random.seed(seed)
        ...
        h_atom_inds = set(
            filter(
                ...
                [i if e == 1 else None for i, e in enumerate(g.v["atomicno"])],
            )
        )
        ...
        non_h_edges = list(
            filter(
                lambda e: (e[0] not in h_atom_inds) and (e[1] not in h_atom_inds),
                src_edges,
            )
        )
        ...
        valid_h_donations = list(
            itertools.product(heavy_atom_inds_with_hs, heavy_atom_inds)
        )
        ...
        for g2 in g2_list:
            ...
            for perm in valid_h_donations:
                ...
                for h_ind in heavy_hs_to_use:
                    g3_h = g3.copy()
                    assert g3_h.adjacency[i, h_ind] == 1, (
                        "h donor must be connected via single-bond"
                    )
                    g3_h.adjacency[i, h_ind] = 0
                    g3_h.adjacency[h_ind, i] = 0

                    assert g3_h.adjacency[j, h_ind] == 0, (
                        "h recipient must not be connected"
                    )
...
class BreakAndRearrangeFast:
    ...
    def __call__(
        ...
        if mol.GetNumAtoms() > 63:
            raise ValueError(
                f" MOlecule has {mol.GetNumAtoms()} atoms, currently we only support up to 63"
            )
        ...
        h_edges = list(
            filter(lambda e: (e[0] in h_atom_inds) or (e[1] in h_atom_inds), src_edges)
        )
        ...
        # generate all valid permutations of hydrogen donations
        valid_h_donations = np.array(
            list(itertools.product(heavy_atom_inds_with_hs, heavy_atom_inds)),
            dtype=np.int64,
        )
        ...
        non_h_remove_subsets = fast.subsets_from_edge_removals(
            non_h_remove_edges,
            g_adj_uint8,
            1,
            valid_h_donations,
        ...
        all_remove_subsets = fast.subsets_from_edge_removals(
            all_remove_edges,
            g_adj_uint8,
            0,
            valid_h_donations,
        ...
        out = fast.array_bitsets_to_array(subsets, n_total_atoms)
        return out
...
def enumerate_break_edges(edge_list, num_breaks):
    M = len(edge_list)
    edgesets_to_break = (
        np.ones((len(edge_list) ** num_breaks + 1, num_breaks, 2), dtype=np.int32) * -1
    )
