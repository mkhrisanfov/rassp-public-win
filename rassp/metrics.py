...
def sdp(
    true: Mapping[float, float], pred: Mapping[float, float], bins=None, max_bin=None
):
    """
...
def intensity_weighted_barcode_false_positive_rate(
    ...
    """
    return 1 - intensity_weighted_barcode_precision(
        d_pred, d_true, frac_true_cutoff, round_pred_masses
    )
...
def wishart_dot_product(
    ...
    for i, mi, wi in zip(range(1, len(pred_masses) + 1), pred_masses, pred_intensities):
        for j, mj, wj in zip(
            range(1, len(true_masses) + 1), true_masses, true_intensities
        ):
            best_val = 0.0
